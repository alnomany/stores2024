...
class ProductController extends Controller
{
    public function index()
    {
        ...
        $getproductslist = Item::with('variation', 'category_info', 'product_image')->where('vendor_id',  $vendor_id)->orderBy('reorder_id')->get();
    ...
    public function add(Request $request)
    {
        ...
        $globalextras = GlobalExtras::where('vendor_id', $vendor_id)->where('is_available', 1)->where('is_deleted', 2)->orderBy('reorder_id')->get();
        ...
        return view('admin.product.add_product', compact("getcategorylist", "globalextras", "gettaxlist"));
    }
    public function save(Request $request)
    {
        ...
        if ($request->attachment_name != "" && $request->attachment_name != null) {
    ...
    public function edit($slug)
    {
        ...
        $globalextras = GlobalExtras::where('vendor_id', $vendor_id)->where('is_available', 1)->where('is_deleted', 2)->orderBy('reorder_id')->get();
        $getproductdata = Item::with('category_info', 'product_image', 'extras')->where('slug', $slug)->first();
        $gettaxlist = Tax::where('vendor_id', $vendor_id)->where('is_deleted', 2)->where('is_available', 1)->orderBy('reorder_id')->get();
        ...
        if (!empty($getproductdata)) {
            ...
            return view('admin.product.edit_product', compact('getproductdata', 'getcategorylist', 'productreview', 'gettaxlist', 'productVariantArrays', 'product_variant_names', 'variant_options', 'globalextras'));
        }
    ...
    public function update_product(Request $request, $slug)
    {
        ...
        try {
            ...
            $product->attchment_name = $request->attachmentname;
            if ($request->attachmentname != "" && $request->attachmentname != null) {
                if ($request->has('attachmentfile')) {
                    ...
                    $reimage = 'attachment-' . uniqid() . "." . $request->file('attachmentfile')->getClientOriginalExtension();
                    $request->file('attachmentfile')->move(storage_path('app/public/admin-assets/images/product/'), $reimage);
                    $product->attchment_file = $reimage;
                }
            }
    ...
    public function update_image(Request $request)
    {
        if ($request->has('product_image')) {
            ...
            ProductImage::where('id', $request->id)->update(['image' => $imgname]);
    ...
    public function delete_product($slug)
    {
        try {
            $checkproduct = Item::where('slug', $slug)->first();
            Variants::where('item_id', $checkproduct->id)->delete();
            Extra::where('item_id', $checkproduct->id)->delete();
            Cart::where('item_id', $checkproduct->id)->delete();
