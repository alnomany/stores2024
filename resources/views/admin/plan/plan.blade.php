...
@section('content')
    <div class="d-flex justify-content-between align-items-center">
        <h5 class="text-uppercase">{{ trans('labels.pricing_plans') }}</h5>
        @if (Auth::user()->type == '1')
            <a href="{{ URL::to('admin/plan/add') }}" class="btn btn-secondary px-2 d-flex"> <i
        ...
        @if (count($allplan) > 0)
            @foreach ($allplan as $plandata)
                <div class="col-xl-3 col-lg-4 col-md-6 col-sm-6 mt-3" data-id="{{ $plandata->id }}">
                    ...
                    @endphp
                    <div
                        class="card border-0 box-shadow h-100 {{ $plan_id == $plandata->id ? 'plan-card-active' : 'border-0' }} handle">
                        <div class="card-body">
                            <div class="d-flex justify-content-between">
                                <h5 class="text-secondary">{{ $plandata->name }}</h5>
                                @if (Auth::user()->type == 1)
                                    <a tooltip="{{ trans('labels.move') }}"><i
                                            class="fa-light fa-up-down-left-right"></i></a>
                                @endif
                            </div>

                            <div class="my-4">
                                <h5 class="mb-2">{{ helper::currency_formate($plandata->price, '') }}
                        ...
                        <div class="card-footer bg-white border-top-0 my-2 text-center">
                            @if (Auth::user()->type == '1')
                                ...
                                <a href="{{ URL::to('admin/plan/edit-' . $plandata->id) }}"
                                    class="btn btn-outline-info btn-sm mt-2" tooltip="{{ trans('labels.edit') }}"> <i
                                        ...
                                        @if ($plandata->price > 0)
                                            ...
                                            @else
                                                @if (@$data['original']['plan_date'] > date('Y-m-d'))
                                                    <small
                                                        class="text-dark d-block">{{ @$data['original']['plan_message'] }}
                                                        : <span
                                                            class="text-success">{{ $data['original']['plan_date'] != '' ? helper::date_format($data['original']['plan_date']) : '' }}</span></small>
                                                @else
                                                    <small
                                                        class="text-dark d-block">{{ @$data['original']['plan_message'] }}
                                                        : <span
                                                            class="text-danger">{{ $data['original']['plan_date'] != '' ? helper::date_format($data['original']['plan_date']) : '' }}</span></small>
                                                @endif
                                            @endif
