...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        <h5 class="text-uppercase">{{ trans('labels.working_hours') }}</h5>
    </div>
    <div class="row mt-3">
                ...
                <form action="{{ URL::to('/admin/time/store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="form-group col-md-12 text-end">
                        <button
                            ...
                            class="btn btn-secondary btn-raised {{Auth::user()->type == 4 ? (helper::check_access('role_working_hours', Auth::user()->role_id, $vendor_id, 'add') == 1  ? '':'d-none'): ''}}">{{ trans('labels.save') }}</button>
                    </div>
                </form>
            </div>
        </div>
