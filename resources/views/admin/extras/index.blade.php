...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <h5 class="text-uppercase">{{ trans('labels.global_extras') }}</h5>

        <a href="{{ URL::to('admin/extras/add') }}"
            class="btn btn-secondary px-2 d-flex {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, $vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}"><i
                                    ...
                                    <tr class="fs-7 row1" id="dataid{{ $extras->id }}" data-id="{{ $extras->id }}">
                                        ...
                                        <td>
                                            @if($extras->is_available == '1')
                                                <a tooltip="{{ trans('labels.active') }}"
                                                    ...
                                                    class="btn btn-sm btn-outline-success {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"><i
                                            ...
                                            @else
                                                <a tooltip="{{ trans('labels.inactive') }}"
                                                    ...
                                                    class="btn btn-sm btn-outline-danger {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}"><i
                                        ...
                                        <td>
                                            <a href="{{ URL::to('admin/extras/edit-' . $extras->id) }}"
                                                ...
                                                class="btn btn-outline-info btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, $vendor_id, 'edit') == 1 ? '' : 'd-none') : '' }}">
                                            ...
                                            <a tooltip="{{ trans('labels.delete') }}"
                                                ...
                                                class="btn btn-outline-danger btn-sm {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, $vendor_id, 'delete') == 1 ? '' : 'd-none') : '' }}">
