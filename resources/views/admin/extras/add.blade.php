...
@section('content')
    <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <nav aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ URL::to('admin/extras') }}">{{ trans('labels.global_extras')
                        }}</a></li>
                    ...
                    <form action="{{ URL::to('admin/extras/save') }}" method="POST">
                        ...
                        <div class="row">
                            ...
                            <div class="form-group text-end">
                                <a href="{{ URL::to('admin/extras') }}" class="btn btn-outline-danger">{{
                                    trans('labels.cancel') }}</a>
                                <button class="btn btn-secondary {{ Auth::user()->type == 4 ? (helper::check_access('role_global_extras', Auth::user()->role_id, Auth::user()->vendor_id, 'add') == 1 ? '' : 'd-none') : '' }}" @if (env('Environment') == 'sendbox') type="button"
